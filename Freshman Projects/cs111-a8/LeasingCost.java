public class LeasingCost {
	...
	public static Vehicle createVehicle(String description){
        ...
        index = description.indexOf("name:");
        String carName = description.substring(index + 5);
        String name = carName.substring(0, carName.indexOf(';'));

        index = description.indexOf("type:");
        String fType = description.substring(index + 5);
        String type = fType.substring(0, fType.indexOf(';'));

        index = description.indexOf("mile/unit:");
        String mPu = description.substring(index + 10);
        String stringUsage = mPu.substring(0, mPu.indexOf(';'));
        double usage = Double.parseDouble(stringUsage);

        Fuel fuel = new Fuel(usage);

        if (type.equals("electric")) {
            index = description.indexOf("charger:");
            String cCharge = description.substring(index + 8);
            String stringCharger = cCharge.substring(0, cCharge.indexOf(';'));
            double charger = Double.parseDouble(stringCharger);

            fuel = new Fuel(usage, charger);
        }

        index = description.indexOf("due:");
        String first = description.substring(index + 4);
        String stringDue = first.substring(0, first.indexOf(';'));
        double due = Double.parseDouble(stringDue);

        index = description.indexOf("allowance:");
        String second = description.substring(index + 10);
        String stringAllowance = second.substring(0, second.indexOf(';'));
        int allowance = Integer.parseInt(stringAllowance);

        index = description.indexOf("length:");
        String third = description.substring(index + 7);
        String stringLength = third.substring(0, third.indexOf(';'));
        int length = Integer.parseInt(stringLength);

        index = description.indexOf("monthly:");
        String fourth = description.substring(index + 8);
        String stringMonthly = fourth.substring(0, fourth.indexOf(';'));
        double months = Double.parseDouble(stringMonthly);

        Lease lease = new Lease(due, length, months, allowance);
	...
	public static void computeCO2EmissionsAndCost( Vehicle[] vehicles, double gasPrice, double electricityPrice ){
        ...
        for(int i = 0; i<vehicles.length; i++) {
            ...
            CO2Emission += computeCO2(lease.getLeaseLength(), fuel.getUsage(), lease.getMileageAllowance(), co2Unit);
            fuelCost += computeFuelCost(lease.getLeaseLength(), fuel.getUsage(), lease.getMileageAllowance(), fuelPrice);
            leaseCost += computeLeaseCost(lease.getDueAtSigning(), lease.getLeaseLength(), lease.getMonthlyCost());


            totalCost = fuelCost + leaseCost + fuel.getCharger();
