...
public class WeatherGenerator {
    ...
    public static void populateLocationProbabilities( double[] drywetProbability, double[] wetwetProbability, double longitude, double latitude, double[][] drywet, double[][] wetwet){
        ...
        for(int i = 0; i < 4100; i++) {
            if(drywet[i][0] == longitude && drywet[i][1] == latitude) {
                for(int j = 2; j <= 13; j++){
                    drywetProbability[k] = drywet[i][j];
                    k++;
                }
            }
        }
        ...
        for(int i = 0; i < 4100; i++) {
            if(wetwet[i][0] == longitude && wetwet[i][1] == latitude) {
                for(int j = 2; j <= 13; j++){
                    wetwetProbability[k] = wetwet[i][j];
                    k++;
                }
            }
        }
    ...
    public static int lengthOfLongestSpell (int[] forecast, int mode) {
        ...
        if(mode == DRY) { 
            for (int i = 0; i < forecast.length-1; i++) {
                if (forecast[i] == forecast[i+1] && forecast[i] == DRY) {
                    count++;
                    if (count >= max) {
                        max = count;
                    }
        ...
        if(mode == WET) {
            for (int i = 0; i < forecast.length-1; i++) {
                if (forecast[i] == forecast[i+1] && forecast[i] == WET) {
                    count++;
                    if (count >= max) {
                        max = count;
                    }
    ...
    public static void main (String[] args) {
        ...
        //StdRandom.setSeed(1667967078632L);
